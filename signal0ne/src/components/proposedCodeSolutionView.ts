...
export async function createProposedCodeSolutionView(explanation: any, 
  issue: any, 
  ...
  Promise<void> {
    ...
    if (!isPanelInit) {
         ...
         null
          ...
          panel.webview.html = getWebviewContent(explanation,issue, accessToken);
          isPanelInit = true;
    } else {
        panel.webview.html = getWebviewContent(explanation, issue, accessToken);
    }
...
function getWebviewContent(explanation: any,issue: any, accessToken: string) {
    return `<!DOCTYPE html>
  <html lang="en">
  <head>
      <meta charset="UTF-8">
