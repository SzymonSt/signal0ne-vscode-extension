...
function getWebviewContent(_issue: any) {
  ...
  <body>
    <div class="container">
        ...
        <p>
        Check the variable 'Effects' in the Main method located in the Program.cs file within the /app directory for null value before using it. Initialize it to an empty collection if it is null. For example: Effects = new List(); EffectSelectorForm effectSelectorForm = new EffectSelectorForm(Effects); This way, even if the variable 'Effects' is null initially, it will be initialized to an empty collection before being passed to the constructor of EffectSelectorForm. This should prevent the NullReferenceException from occurring.
        </p>
        <h2>Sources: </h2>
        <a class="source-link" href="https://stackoverflow.com/questions/1943465/avoiding-null-reference-exceptions" target="blank">https://stackoverflow.com/questions/1943465/avoiding-null-reference-exceptions",</a>
    </div>
